...
namespace CourseProfessorAllocation
{
    class Program
    {
        ...
        {
            MyDb db = new MyDb();
            Course course = new Course();  // instantiating the class
            Topic topic = new Topic();
            Expertise expert = new Expertise();
            Professor professor = new Professor();
            CourseTopic courseTopic = new CourseTopic();
           
            List<Professor> professors = db.GetProfessors(); // each method will return the list from db
            List<Course> courses = db.GetCourses();
            List<Topic> topics = db.GetTopics();
            List<Expertise> experts = db.GetExpertise();
            List<CourseTopic> courseTopics = db.GetCourseTopics();  // returns the list with course id,topic id ans its weightage percentage
        ...
        private static void PrintData(List<Course> courses, List<Topic> topics, List<Professor> professors, List<Expertise> experts, List<CourseTopic> courseTopics)
        {
            ...
            for (int courseIndex = 1; courseIndex <= courses.Count; courseIndex++)
            {
                ...
                foreach (var ct in courseTopics.Where((ct) => ct.CourseId == courseIndex)) // we will iterate courseTopics list and print the raw data
                {
